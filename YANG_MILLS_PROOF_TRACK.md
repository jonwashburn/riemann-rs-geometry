...
- **2025-12-24**: Upgraded Track-J U1-J1 citation to theorem-number-grade: `lem:heatkernel-lsi-compactG` proof now cites Bakry–Émery \cite[Prop.~3]{BakryEmery1985} (curvature-dimension criterion for LSI on compact manifolds), Gross \cite[Thm~1, Cor~4]{Gross1975} (hypercontractivity–LSI equivalence), and Holley–Stroock \cite[Prop.~4.3]{HolleyStroock1987} (bounded perturbation lemma). Checked off the U1-J1 checkbox in the Track J section. Track J is now fully closed for the tightness route (both U1-J1 and U1-J2 checked).
- **2025-12-24 (continued)**: Audited OS1 calibrator route. Added explicit citation to `lem:U1-tree-bounds` in the proof of `prop:hk-calibrators-constructed` and stated the heat-kernel approximation estimate with explicit constant dependence on \((R,G)\). Added and checked off the calibrator-audit checkbox in the OS1 section.
- **2025-12-25**: Audited and corrected `thm:U2-nrc-unique`: fixed a sign error in the resolvent identity and its derived “one-point ⇒ compact $K$” bootstrap (changed `(w-z)` to `(z-w)` and corrected the algebra to use \(I-\delta R(z_0)\) with \(\delta=z-z_0\)).
- **2025-12-25 (continued)**: Audited `prop:one-point-resolvent`: removed the missing-projector-term gap by making the proof explicitly reduce to Theorem~`thm:nrc-quant` (and its remark) for the $O(a)$ one-point bound at $z_0$.
- **2025-12-25 (continued)**: Fixed a real correctness issue in `lem:U2-defect-core`: removed an incorrect “$\int_0^\infty$ semigroup identity” and replaced it by the correct generator/derivative-at-$0^+$ identity, and corrected a mis-citation in `thm:abstract-nrc` (comparison identity now points to `lem:U2-comparison`).
- **2025-12-25 (continued)**: Audited low-energy projector control: fixed a constant mismatch in `lem:low-energy-proj` (statement now matches its contour proof, using the `C_D` constant from `lem:graph-defect-Oa`) and tightened `thm:nrc-quant` to assume the necessary spectral-gap condition \(g=\mathrm{dist}(\Lambda,\sigma(H))>0\) (since the proof uses contour/Riesz projection).
- **2025-12-25 (continued)**: Found and fixed a major U2 citation/assumption mismatch: `thm:nrc-quant` and `prop:one-point-resolvent` were citing `thm:quant-calibrated-af-free-nrc}(D)` as a generator graph-defect bound (it is not; (D) is an embedding/inner-product defect). We made `thm:nrc-quant` explicitly **conditional** on (i) low-energy projection control \(\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a\) and (ii) a graph-defect bound \(\|D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a\), and removed the incorrect citations from `prop:one-point-resolvent`, `lem:af-free-cauchy`, and `prop:resolvent-comparison`. These two U2 inputs are now the next explicit blockers to either prove or isolate as named assumptions.
- **2025-12-25 (continued)**: Added a named assumption bundle `assump:U2b` in `Yang-Mills.tex` and rewired `thm:nrc-quant`, `prop:one-point-resolvent`, and `lem:af-free-cauchy` to reference it (so the conditionality is centralized and unmistakable).
- **2025-12-25 (continued)**: Tightened the high-level dependency statements (`Referee Quick-Check`, `thm:global-OS`, `thm:main-af-free`) so they cite `Assumption~\ref{assump:U2b}` explicitly and do not describe U2 as “proved” when U2b is still an RG-grade conditional input.
- **2025-12-25 (continued)**: Fixed a notation collision that could mislead referees: in `thm:quant-calibrated-af-free-nrc`, renamed the *embedding/isometry defect* from `D_a:=I_a^*I_a-\mathbf 1` with constant `C_D` to `\Delta_a^{\rm iso}` with constant `C_{\rm iso}`, and updated the downstream uses of the Davis–Kahan constant bound (`C_{\mathrm{NRC}}\le 2C_{\rm form}+4C_{\rm iso}^2`). This prevents confusion with the U2 generator-defect operator \(D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}\) and its constant.
- **2025-12-25 (continued)**: “Unconditional vs conditional” audit pass: updated the Abstract, `thm:global-gap-operator`, `thm:global-gap-uncond`, `thm:clay-compliance`, and `cor:os-assumptions-verified` so the continuum statements are explicitly **conditional** on the fixed-region inputs and, in particular, `Assumption~\ref{assump:U2b}` (removing several overclaims of “U2 proved” / “unconditional” continuum gap).
- **2025-12-25 (continued)**: Extended the same audit to the “Clay–critical consolidated theorems” block (`thm:global-os-clay`, `thm:global-nrc-clay`, `thm:gap-wightman-clay`, `thm:independence-clay`) and the global gap corollary (`cor:global-uniform-gap`): marked them explicitly **conditional** (and corrected “$\varepsilon(a)=O(a)$ in general” to “under `Assumption~\ref{assump:U2b}`” where it appeared in the NRC constants box).
- **2025-12-25 (continued)**: Added an explicit “status/analogy” remark under `assump:U2b` pointing to the proved fixed-slab $J_a$ analogues (`lem:graph-defect-Oa`, `lem:low-energy-proj`), and updated the intro Scope paragraph to cite `assump:U2b` directly (so the conditionality is visible before the technical sections).
- **2025-12-25 (continued)**: Cleaned up a remaining “hidden U2b proof” regression: `thm:nrc-embeddings` and the proof of `lem:af-free-cauchy` were still informally deriving the embedding-side graph-defect / projector control from “locality + UEI”. These have been rewritten so `thm:nrc-embeddings` is explicitly **conditional on `Assumption~\ref{assump:U2b}`**, and the proofs no longer cite the fixed-slab $J_a$ analogues as if they implied the OS/GNS embedding-side bounds.
- **2025-12-25 (continued)**: Removed the largest remaining overclaim: `thm:nrc-explicit` previously asserted “NRC for all nonreal $z$ (no hypothesis)” and attempted to derive the U2b graph-defect / projector control from UEI/locality. It is now explicitly **conditional on `Assumption~\ref{assump:U2b}`** and reduced to `thm:nrc-operator-norm`; likewise `prop:collective-compactness` and `thm:nrc-operator-norm` are now labeled conditional in their statements.
- **2025-12-25 (continued)**: Fixed a visible honesty bug in the main theorem section heading: changed “AF--free NRC with **Proved** UEI/OS1 inputs” to “AF--free NRC with fixed-region inputs” (section label unchanged), to match the manuscript’s conditional dependency story.
- **2025-12-25 (continued)**: Removed a confusing “unconditional” tag from the fixed-slab contour projector lemma title (`lem:low-energy-proj`), since the statement already has an explicit spectral-gap-at-cutoff hypothesis \(g=\mathrm{dist}(\Lambda,\sigma(H))>0\).
- **2025-12-25 (continued)**: Honesty polish in an optional (non-mainline) section: updated the KP/area-law scaling-window intro sentence to say it yields a **conditional** continuum theory (since it assumes a uniform KP window), avoiding “fully rigorous continuum theory” phrasing without qualifiers.
- **2025-12-25 (continued)**: Interface-smoothing honesty sweep: rewired the front-matter summaries and the Clay/global theorem pack to treat **UCIS$_{\rm SW}$** (under `eq:ucis-sw-window`, with bookkeeping via `prop:os35-limit`) as the active interface contraction input, and explicitly marked the one-step coarse-refresh route as **legacy / not used in the main chain**. Removed remaining “$\beta$-uniform after coarse refresh” overclaims from load-bearing summaries (including `cor:global-uniform-gap` / `thm:global-minkowski-gap`) and pointed them to `thm:global-gap-uncond` + OS→Wightman export instead.
- **2025-12-25 (continued)**: Cleaned up two additional front-facing consistency hazards: (i) demoted the *early* E1/E2 “global OS / global NRC / Wightman export / independence” theorem pack (`thm:global-os3-clustering`, `thm:global-OS0-5`, `thm:nrc-global-Oa2`, `thm:os-wightman-export`, `thm:global-independence`) to **conditional pointers** to the Clay pack (and removed the stale “global $O(a^2)$ NRC” overclaim), and (ii) updated the front-matter “Minimal Chain” + constants table to match UCIS$_{\rm SW}$ + `assump:U2b` (no residual one-step/β-uniform Doeblin story in the main chain). Also introduced a fixed-region defect assumption `assump:U2bR` (replacing the earlier “proved” `lem:graph-defect-core`) and stated a named Bridge Lemma target `lem:bridge-U2b-target` under `assump:U2b`.
- **2025-12-25 (continued)**: TeX + consistency hardening: `pdflatex -halt-on-error Yang-Mills.tex` now succeeds again after (i) removing sectioning commands inside framed boxes (two `\subsubsection*` callsites inside `mdframed`), (ii) fixing a malformed “$*\,--\,algebra$” string that had entered math mode incorrectly, and (iii) extending appendix section numbering beyond `Z` with an expandable `AA, AB, ...` macro (avoids “Counter too large” failures on long appendices). Also patched leftover $\beta$-independence overclaims outside the UCIS$_{\rm SW}$ main chain by rewriting `prop:explicit-doeblin-constants` as a $\beta$-explicit one-step weight (pointing to `prop:doeblin-full`), updating derived-constants notes, and marking the $\beta$-independent interface-minorization appendix as a legacy proof sketch (not used in the main chain).
- **2025-12-25 (continued)**: U2 fixed-region cleanup + bridge sharpening: rewrote the fixed-region NRC auxiliary block to be internally consistent with the global comparison-identity approach (updated `assump:U2bR` to a two-part fixed-region analogue of `assump:U2b`, rewrote `thm:nrc-operator-norm-fixed`, and removed the now-redundant fixed-region NRC corollaries). Sharpened the RG-grade blocker `lem:bridge-U2b-target` to list explicit quantitative bridge inequalities (B1–B2; the comparison low-energy piece is now supplied separately by `lem:cell-avg-lowenergy`) rather than informal “stable under inserting weights” language. `pdflatex` still succeeds.
- **2025-12-25 (continued)**: NRC honesty tightening: removed an over-optimistic “choose $\Lambda(a)$ slowly ⇒ $O(a)$” claim from `thm:nrc-quant` (now phrased as a conditional implication requiring control of $C(z,\Lambda(a))$), rewrote `prop:one-point-resolvent` to the correct cutoff-form bound (includes the explicit high-energy tail term), and updated the “explicit constant” remark accordingly.
- **2025-12-25 (continued)**: Uniqueness-chain honesty: added an explicit “$\to 0$ as $a\downarrow 0$” note to `prop:one-point-resolvent` (via the standard “choose $\Lambda$ then choose $a$” argument), and made `cor:unique-schwinger-local` explicitly conditional (now stated in terms of `thm:U2-nrc-unique`/`assump:U2b` rather than reading as unconditional).
- **2025-12-25 (continued)**: Removed a hidden load-bearing overclaim in the “optional AF schedule” block: deleted `lem:cauchy-res` (standalone $O(a+b)$ Cauchy bound) and rewrote the C1d section as a clearly labeled conditional packaging (not an AF existence proof). Downstream schedule-independence is now routed through `prop:one-point-resolvent` + `thm:U2-nrc-unique` (conditional on `assump:U2b`).
- **2025-12-25 (continued)**: Global-construction honesty tightening: rewrote `thm:tightness-beta` / `cor:global-convergence` so tightness/precompactness is separate from uniqueness (now explicitly conditional on the AF--free NRC/uniqueness package, hence `assump:U2b`), demoted the “AF-free calibrated NRC alternative” as optional, and rewrote `thm:c1-consolidated` as an optional Mosco cross-check asserting only subsequential limits. Also updated Appendix U’s referee checklist to reference UCIS$_{\rm SW}$ (not a $\beta$-uniform one-step Doeblin story), and rewired nontriviality callsites: `prop:nonzero-cumulant4`, `cor:nonGaussian-main`, and `lem:nontrivial-variance` are now explicitly conditional and no longer rely on the old Appendix C1a uniqueness statement or a bogus uniform-variance-from-one-step-Doeblin argument. Finally, simplified `prop:field-closability` to use uniform $L^\infty$ bounds of smeared plaquette/clover observables (bounded multiplication operators).
- **2025-12-25 (continued)**: Bridge lemma tightening: added `lem:cell-avg-lowenergy` (cell-averaging $J_a^*J_a$ is $O(a)$-close to the identity on low-energy spectral subspaces), and removed (B3) as an extra assumption from `lem:bridge-U2b-target` (now B3 is supplied by the new lemma; remaining RG-grade bridge inputs are the true quasi-unitary matching conditions (B1)–(B2)).
- **2025-12-25 (continued)**: Bridge lemma hygiene: renamed the bridge-unitary to `V_{a,L}` (to avoid collision with the interface density isometry `U_{a,L}` used later), and tightened the proof-sketch wording to use the identity $(J_a^*V_{a,L})(J_a^*V_{a,L})^*=J_a^*J_a$ (instead of incorrectly calling $J_a^*V_{a,L}$ an “isometry”).
- **2025-12-25 (continued)**: Fixed a hard correctness mismatch in `DEC:plaquette-F2`: the statement was a classical plaquette/curvature approximation but the proof was an unrelated LSI/UEI argument. Rewrote the proof as a standard holonomy/BCH expansion (citing \cite{MontvayMunster1994}), and made the needed normalization and technical hypotheses explicit (continuum normalization written directly in $\beta$: $S_U(A)=\frac{\beta}{4N}\int_U \Tr(F_{\mu\nu}F_{\mu\nu})\,dx$, plus a collar/support assumption to suppress boundary-layer discretization artifacts). `pdflatex -halt-on-error Yang-Mills.tex` still succeeds.
- **2025-12-25 (continued)**: Honesty cleanup in a non-mainline section: the “AF--free: Time-Slice $O(a^2)$ Control and NRC (Auxiliary Outline)” paragraph incorrectly said its bullets were “used unconditionally in the main line”. Patched the TeX to say it is an auxiliary outline only, and explicitly point the main NRC chain to the comparison identity + `Assumption~\ref{assump:U2b}`.
- **2025-12-25 (continued)**: Sharpened the remaining U2b Bridge Lemma blocker: expanded the remark after `lem:bridge-U2b-target` to give an explicit \emph{form-level graph-boundedness criterion on an operator core} for (B2) (a bilinear estimate for $V_{a,L}^*H_aV_{a,L}-H_{a,L}$ relative to the $(H_{a,L}+1)^{1/2}$ norm, tied conceptually to the `NRC:form` framework but not overstating an implication from abstract form convergence alone), and clarified that (B1) reduces to a deterministic geometric approximation between the polygonal/smoothed embedding and the piecewise-constant cell extension on the time-zero local core. `pdflatex` still succeeds.
- **2025-12-25 (continued)**: Title/front-matter honesty: updated the title and PDF metadata title to say “\emph{Conditional} AF--free continuum construction,” matching the abstract and the global conditionality on fixed-region inputs (UCIS$_{\rm SW}$ + U1/OS1 + Assumption~\ref{assump:U2b}).
- **2025-12-25 (continued)**: Bridge-remark and header hygiene: softened the Bridge Lemma “form-level reduction” phrasing to avoid overstating that a form-bilinear estimate automatically implies (B2), and updated the running short title to include “conditional AF--free continuum,” matching the abstract/title.
- **2025-12-25 (continued)**: Bridge lemma usability: added a short remark item explaining a practical way to \emph{construct} the bridge unitary $V_{a,L}$ on the time-zero local core when both models are OS/GNS completions of the same generator algebra (define on generators and extend as in `prop:unitary-equivalence`); quantitative bounds then reduce to controlling inner products/forms on that core, aligning with (B1)--(B2).
- **2025-12-25 (continued)**: Comparison-model hygiene in the U2 slab template: rewrote the “U2 on fixed slabs” block (`thm:U2`, `lem:U2-electric`, `lem:U2-magnetic`, and the note after `lem:graph-defect-Oa`) to consistently present it as a \emph{classical} $L^2/\ell^2$ discretization comparison model (no OS/GNS wording, no “van Hove/volume” phrasing), and explicitly point out that passing from such comparison-model bounds to the OS/GNS embedding-side defect estimate is exactly the RG-grade bridge/U2b blocker.
- **2025-12-25 (continued)**: Minor clarity follow-up: removed the use of the OS/GNS notation $\mathcal D^{\rm loc}$ inside the comparison-model proofs of `lem:U2-electric`/`lem:U2-magnetic` (now described simply as a smooth comparison-model core like $C_c^\infty(\Omega_T)$), to prevent readers from thinking those lemmas are stated on the quantum OS/GNS domain.
- **2025-12-25 (continued)**: OS1 commutator-route hygiene: removed a duplicated subsection header, marked the commutator-route criterion (`thm:emergent-sym`) and its supporting steps (`lem:local-commutator-Oa2`, `prop:resolvent-from-commutator`) explicitly as \emph{optional cross-checks} (and made the dependence on the outline-only sandwich input `TS:sandwich_main` explicit). Also renamed the embedding notation in `thm:emergent-sym`/`prop:resolvent-from-commutator` from `J_a` to `I_{a,L}` to avoid collision with the comparison-model cell-averaging injection `J_a`. `pdflatex` still succeeds.
- **2025-12-25 (continued)**: Bridge lemma notation hygiene: renamed the Bridge Lemma’s comparison-model Hilbert space/operator from $(\mathcal H_a,H_a)$ to $(\mathcal H_a^{\rm cmp},H_a^{\rm cmp})$ to avoid collisions with other uses of $\mathcal H_a,H_a$ in the paper. Also rewrote the “constructing $V_{a,L}$” remark to be accurate even when the comparison model is not itself an OS/GNS completion: the missing content is a \emph{canonical} identification of dense time-zero local cores plus quantitative inner-product/form control (exactly (B1)–(B2)).
- **2025-12-25 (continued)**: Bridge lemma consistency sweep: eliminated a residual $(J_a^*,H_a)$ reference in `lem:bridge-U2b-target` (now consistently $(J_a^*,H_a^{\rm cmp})$) and confirmed the main comparison-model analogues are only ever cited as \emph{analogues}, never as implied proofs of OS/GNS `assump:U2b` without the bridge.
- **2025-12-25 (continued)**: TeX/referee hygiene: labeled the previously “floating” proof block after `thm:nrc-embeddings` as an explicit \emph{Proof sketch of Theorem~\ref{thm:nrc-embeddings}} (so it’s clear what it is proving even though a lemma intervenes), and removed a duplicated “Step 2 (density and extension)” paragraph in the proof of `thm:pf-gap-meanzero` while keeping the full conclusion.
- **2025-12-25 (continued)**: Normalization tripwire fix: corrected the one-tick transfer normalization in `thm:nrc-embeddings` from $T_{a,L}=e^{-H_{a,L}}$ to the correct $T_{a,L}=e^{-aH_{a,L}}$ (consistent with the global convention $T=e^{-aH}$ and $H=-a^{-1}\log T$ elsewhere). `pdflatex` still succeeds.
- **2025-12-25 (continued)**: Referee tripwire cleanup: removed the redundant/unproved numbered statement `prop:resolvent-comparison` (it duplicated the fully proved comparison identity `lem:U2-comparison`), so the NRC proof sketch no longer contains a “proposition with no proof”. `pdflatex` still succeeds.
- **2025-12-25 (continued)**: TeX packaging hardening: fixed a missing-citation-key mismatch (`\cite{OsterwalderSeiler1978}` now matches `\bibitem{OsterwalderSeiler1978}`), and made the `fancyhdr` footer degrade gracefully when `lastpage.sty` is unavailable (avoids `\pageref{LastPage}` undefined-reference warnings on minimal TeX installs).
- **2025-12-25 (continued)**: Clean compile output: removed hyperref “Token not allowed in a PDF string” warnings by sanitizing section/subsection headings and a few theorem titles (replacing math/arrows/`\,,--\,` constructs in headings with bookmark-safe text), then reran `pdflatex` until cross-references/outlines stabilized. Current `pdflatex -halt-on-error` run has **no undefined references/citations** and no hyperref PDF-string warnings (only benign underfull boxes remain).
- **2025-12-25 (continued)**: Added an internal workflow note `U2_OPERATIONAL_PLAN.md` that spells out the remaining U2b bridge work (constructing/estimating the canonical local unitary $V_{a,L}$ and proving (B1)–(B2) in `lem:bridge-U2b-target`). Updated `YANG_MILLS_SUBMISSION_CHECKLIST.md` Step 0 to reference it.
- **2025-12-25 (continued)**: Bridge lemma proof polish: expanded the low-energy projector part of the proof sketch in `lem:bridge-U2b-target` into an explicit norm bound, showing \(\|(P_{a,L}-J_a^*J_a)E_H([0,\Lambda])\|\lesssim C_{\rm br}\sqrt{\Lambda+1}\,a\) (using the (B1) adjoint-weighted estimate) and hence an explicit admissible choice of the resulting \(C_\Lambda\).
- **2025-12-25 (continued)**: Graph-defect hygiene upgrade: refactored the U2b (and U2b$_R$) defect input to the two-sided graph-weighted form \(\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\lesssim a\) (and similarly on fixed regions). This matches the proved comparison-model defect estimate `lem:graph-defect-Oa`, avoids implicit unbounded-operator steps, and leaves the NRC constants bookkeeping unchanged (defect term now pairs with the standard weighted resolvent bounds for both $H$ and $H_{a,L}$). Clean `pdflatex -halt-on-error` still succeeds.
- **2025-12-25 (continued)**: Bridge lemma consistency fix after the two-sided defect refactor: strengthened (B1) in `lem:bridge-U2b-target` to include the missing $(H+1)^{1/2}$ / $(H_{a,L}+1)^{1/2}$ graph-control needed to actually imply the two-sided U2b defect bound, and expanded the defect part of the proof sketch into an explicit decomposition with term-by-term $O(a)$ bounds (comparison defect + embedding remainders + generator match). `pdflatex -halt-on-error` still succeeds.
- **2025-12-25 (continued)**: Bridge lemma proof-sketch correctness upgrade: fixed the low-energy projector-control part of `lem:bridge-U2b-target` to avoid domain-mismatched compositions (use $(I-J_a^*V_{a,L})I_{a,L}^*E_H$ rather than an ill-typed $(I-J_a^*V_{a,L})E_H$), and added the missing explicit boundedness ingredients needed for that estimate: a comparison-model graph bound for $J_a$ (constant $C_J$) and an explicit graph-norm stability bound for $V_{a,L}$ (constant $C_{\rm eq}$, now included in (B2)). Updated `U2_OPERATIONAL_PLAN.md` accordingly. `pdflatex -halt-on-error` still succeeds.
- **2025-12-25 (continued)**: Notation collision fix: renamed the Bridge Lemma’s new constants to avoid clashes with existing uses of $C_{\rm eq}$ (equicontinuity modulus) and $C_J$ (earlier Jacobian/polar-decomposition constant). The bridge now uses $C_{\rm gr}$ for the $V_{a,L}$ graph-norm stability and $C_{\rm inj}$ for the $J_a$ graph-boundedness constant.
- **2025-12-25 (continued)**: Added a short deterministic functional-analytic template for the “canonical unitary” part of the bridge: Lemma `lem:bridge-polar-unitary` (polar decomposition upgrades a near-isometry $T$ to an isometry/unitary with explicit operator-norm control). This makes the “construct $V_{a,L}$ on the local core” step auditable once a near-isometric identification map is produced.
- **2025-12-25 (continued)**: Bridge lemma usability: added a short remark after `lem:bridge-polar-unitary` spelling out the standard construction flow \(T_{a,L}\leadsto V_{a,L}\) (near-isometry on the local core, polar decomposition, then reduce (B1)–(B2) to estimates for \(T_{a,L}\) and the generator defect \(T_{a,L}H_{a,L}-H_a^{\rm cmp}T_{a,L}\)). This keeps the remaining RG-grade work explicit without pretending it is done.
- **2025-12-25 (continued)**: Minor honesty tweak: clarified in the remark after `lem:bridge-polar-unitary` that polar decomposition is a deterministic tool to enforce the “unitary/isometry” aspect once a canonical candidate map is available, but that it does **not** by itself preserve the generator-match estimate (B2) without additional commutator/regularity input on the local core. Updated `U2_OPERATIONAL_PLAN.md` to match this (B1 is naturally reduced via $T_{a,L}$; (B2) is usually verified for the final $V_{a,L}$ directly).
- **2025-12-25 (continued)**: Bridge lemma concreteness upgrade: expanded the “construct $V_{a,L}$” remark in `lem:bridge-U2b-target` with an explicit template construction when both models admit a tree-gauge $L^2$ realization on the same chord-variable manifold: define $V_{a,L}$ as the density isometry $f\mapsto f\sqrt{d\mu_{a,L}/d\mu^{\rm cmp}_a}$. This makes the remaining RG-grade content of (B1)–(B2) maximally checkable (reduce to quantitative inner-product / Dirichlet-form control of the density ratio on a common cylinder core).
- **2025-12-25 (continued)**: Promoted the same idea to a labeled target lemma: `lem:bridge-density-isometry-target` (tree-gauge $L^2$ density-isometry bridge criterion). This gives referees a single place to see what concrete objects/estimates would certify (B1)–(B2) in a tree-gauge $L^2$ realization.
- **2025-12-25 (continued)**: Strengthened `lem:bridge-density-isometry-target` into an explicit “sufficient conditions” checklist: (D1) $L^\infty$ control of the log density ratio $\log(d\mu_{a,L}/d\mu_a^{\rm cmp})=O(a)$, (D2) a form-level comparison inequality for the Dirichlet forms under the density isometry (the concrete analytic content of (B2)), and (D3) the deterministic geometric embedding comparison needed for (B1). This keeps the remaining U2b bridge work honest and finitely checkable.
- **2025-12-25 (continued)**: Tightened the checklist so (D2) is \emph{literally} the same bilinear $(H+1)^{1/2}$ graph-form estimate displayed in the Bridge Lemma remark (with the correct inner products on $L^2(\mu_{a,L})$ and $L^2(\mu^{\rm cmp}_a)$), and expanded (D1) to record the immediate consequence $\|V_{a,L}-\mathbf 1\|\lesssim a$ for the density isometry. This removes any ambiguity about what “prove (D2)” means for (B2).
- **2025-12-25 (continued)**: Made the geometric item (D3) equally literal: it is now written as the \emph{exact four operator inequalities} in (B1) (ambient + $(H+1)^{1/2}$-weighted, plus the adjoint pair with $(H_{a,L}+1)^{1/2}$), with a single constant $C_{\rm geom}$. This pins the “geometry work” down to a finite, directly-quotable target.
...
**Current single highest-impact blocker (as of 2025-12-25):** U2 quantitative inputs needed for operator-norm NRC are not yet proved in the OS/GNS embedding framework:
- **(U2b) Graph-defect bound** for \(D_{a,L}:=H I_{a,L}-I_{a,L}H_{a,L}\): \(\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a\).
- **(U2b) Low-energy projection control** for \(P_{a,L}:=I_{a,L}I_{a,L}^*\): \(\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a\).
These are now explicitly surfaced as assumptions in `thm:nrc-quant` / `prop:one-point-resolvent`. The precise missing “Bridge Lemma” statement is now recorded in-text as `lem:bridge-U2b-target` (under `assump:U2b`); the next step is to either prove it (preferred) or keep it as the explicit RG-grade blocker.
...
- [x] **Track J (creative bypass; labeled targets)**:
  - [x] **U1-J1 (heat-smoothing ⇒ dimension-free LSI/Herbst)**: `lem:U1-J1-smoothed-lsi` (reduced to `lem:heatkernel-lsi-compactG` + tensorization/entropy chain rule). **Citation now theorem-number-grade (2025-12-24)**: Bakry–Émery \cite[Prop.~3]{BakryEmery1985}, Gross \cite[Thm~1, Cor~4]{Gross1975}, Holley–Stroock \cite[Prop.~4.3]{HolleyStroock1987}.
...
- [x] **Audit `prop:hk-calibrators-constructed` for explicit citations (2025-12-24)**: proof now cites `lem:U1-tree-bounds` for tree–gauge Lipschitz bounds and states the heat-kernel approximation estimate explicitly with constants depending only on \((R,G)\).
- [x] **Audit `thm:os1-calibrator-route` for explicit citations (2025-12-24)**: proof properly cites `Proposition~\ref{prop:hk-calibrators-constructed}` and uses UEI/equicontinuity (from U1) for precompactness. Proof logic is referee-grade.
- [ ] **(Optional cross-check)** Define the lattice action of rigid motions \(U_a(G)\) (STD): specify how \(G\in E(4)\) acts on lattice cylinders/loops and prove \(U_a(G)\) is unitary on the OS/GNS quotient and compatible with embeddings.
...
- [x] **Prove `DEC:plaquette-F2` with explicit \(O(a^2)\) control (hard STD)**: finite-region Taylor control needed to compare magnetic terms under small rotations/relabellings (and to justify the “finite-stencil” estimates invoked in the commutator proof). ✅ (2025-12-25: proof rewritten as a classical plaquette holonomy expansion with explicit normalization; now matches the statement.)
...
- [x] **Embeddings / partial isometries (STD)**: `lem:U2-embeddings` provides the OS/GNS isometry extension; additionally, `thm:U2-nrc-unique` statement was tightened to remove the unused “intertwine time translations” phrase (2025-12-25).
- [x] **Common core + defect operator (STD)**: `lem:U2-defect-core` is now correct (no erroneous $\int_0^\infty$ identity) and states the generator/derivative-at-$0^+$ identity (2025-12-25).
- [ ] **Graph-defect estimate (RG)**: prove the two-sided graph-weighted bound \(\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C a\) uniformly in \(L\). This is isolated in TeX as Assumption `assump:U2b`(i) and is blocked by the quantitative bridge `lem:bridge-U2b-target` (RG-grade).
- [ ] **Low-energy projector control (RG)**: the contour lemma `lem:low-energy-proj` controls a *different* projector conjugation shape (via $J_a$) than the embedding-side quantity \(\|(I-P_{a,L})E_H([0,\Lambda])\|\) used in `thm:nrc-quant`; this embedding-side control is now an explicit U2b blocker/assumption (2025-12-25).
- [x] **Uniform resolvent–graph bounds (STD)**: `lem:weighted-resolvent` proves the lattice-side bound uniformly in $(a,L)$; the same spectral-theorem estimate gives the continuum-side bound for $H$ (2025-12-25).
- [ ] **One-point resolvent estimate at \(z_0\) (STD)**: now explicitly conditional on the U2b inputs surfaced in `thm:nrc-quant` / `prop:one-point-resolvent` (2025-12-25).
- [x] **Bootstrap from \(z_0\) to compact \(K\subset\mathbb C\setminus\mathbb R\) (STD)**: `thm:U2-nrc-unique` now has a correct resolvent-identity bootstrap (2025-12-25 sign fix), yielding \(\sup_{z\in K}\|R(z)-I_{a,L}R_{a,L}(z)I_{a,L}^*\|\to 0\) from the one-point bound at \(z_0\).
...
| 24 | `thm:nrc-quant` / `prop:one-point-resolvent` | conditional (U2b quantitative inputs not yet proved) | U2b graph-defect + U2b low-energy projection control | We corrected the internal mis-citation (do **not** cite `thm:quant-calibrated-af-free-nrc}(D)` as a generator graph-defect). The embedding-side quantitative inputs are now centralized and made explicit in TeX as Assumption `assump:U2b`: (i) the two-sided graph-weighted defect bound \(\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a\) and (ii) the low-energy projection control \(\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a\). The remaining real work is to actually **prove** these (RG-grade), e.g. via the quantitative bridge target `lem:bridge-U2b-target`. |
| 25 | `thm:quant-calibrated-af-free-nrc` (D) | terminology hazard (fixed locally; watch callsites) | — | In this theorem, (D) is an embedding/inner-product defect (`I_a^*I_a-\mathrm{id}`), not the generator graph-defect (`H I-IH_a`). We patched downstream NRC callsites in the U2 appendix to stop using it as a graph-defect bound; keep an eye out for new callsites that reintroduce this confusion. |
| 26 | `assump:U2b` | RG-grade missing bridge lemma (BLOCKED) | quantitative identification between OS/GNS embeddings \(I_{a,L}\) and a concrete discretization/comparison model | **Crisp missing statement (what would actually prove U2b):** prove the in-text target `lem:bridge-U2b-target` (under `assump:U2b`) with explicit quantitative quasi-unitary equivalence data. Concretely: produce unitaries \(V_{a,L}:\mathcal H_{a,L}\to\mathcal H_a^{\rm cmp}\) to a comparison discretization \((\mathcal H_a^{\rm cmp},H_a^{\rm cmp},J_a)\) (e.g. the fixed-slab \(J_a\) model in Prop.~`lem:graph-defect-Oa`/Cor.~`lem:low-energy-proj`) such that, uniformly in \(L\), one has **(B1)** the embedding-match bounds (both ambient and energy-graph weighted) in `lem:bridge-U2b-target`, and **(B2)** \(\|(V_{a,L}H_{a,L}-H_a^{\rm cmp}V_{a,L})(H_{a,L}+1)^{-1/2}\|\le C_{\rm br,H}a\). The remaining “comparison low-energy projection” ingredient is supplied in-text by `lem:cell-avg-lowenergy` (so it is no longer an extra bridge assumption). Together with the proved comparison defect/projector bounds, this upgrades to the embedding-side U2b bounds \(\|(H+1)^{-1/2}D_{a,L}(H_{a,L}+1)^{-1/2}\|\le C_{\rm gd}a\) and \(\|(I-P_{a,L})E_H([0,\Lambda])\|\le C_\Lambda a\). **Status:** target statement sharpened in TeX; remains unproved (still the load-bearing RG-grade blocker). |
...
| YM-0005 | theorem | `thm:quant-calibrated-af-free-nrc` | Calibrated fixed-slab NRC template (optional; legacy one-step Doeblin route) | Introduction / Main Statements (Lattice, Small $\beta$) | 393-465 | S0 | C0 | Updated (2025-12-25): demoted/relabeled in TeX as an optional calibrated template (not used in the main UCIS$_{\rm SW}$ chain). |
...
| YM-0023 | corollary | `cor:semigroup-on-compact` | (Removed) Operator--norm semigroup convergence on compact times | Introduction / Main Statements (Lattice, Small $\beta$) / Constant Ledger (explicit dependencies) | 729-734 | — | — | Removed from `Yang-Mills.tex` (2025-12-25) as an unused auxiliary corollary tied to an earlier fixed-region NRC block. |
...
| YM-0039 | proposition | `prop:nonzero-cumulant4` | Non-Gaussianity: nonzero truncated 4-point for local fields (conditional) | Appendix: Global Constants Table (Provenance) | 975-981 | S0 | C0 | Updated (2025-12-25): now explicitly conditional (uses `cor:unique-schwinger-local` / `assump:U2b`), and no longer cites `thm:c1a-tight`. |
...
| YM-0046 | assumption | `assump:U2bR` | U2b$_R$: quantitative NRC inputs on fixed regions (RG-grade) | Appendix: Global Constants Table (Provenance) | 1116-1123 | S0 | C0 | Updated (2025-12-25): now matches global `assump:U2b` structure (graph-defect + low-energy projection control) on fixed regions. |
| YM-0047 | lemma | `lem:davis-kahan-fixed` | (Removed) Low-energy projectors: Davis--Kahan on fixed regions | Appendix: Global Constants Table (Provenance) | 1171-1180 | — | — | Removed from `Yang-Mills.tex` (2025-12-25) when refactoring the fixed-region NRC block to match the global comparison identity approach. |
| YM-0048 | theorem | `thm:nrc-operator-norm-fixed` | Quantitative operator--norm resolvent comparison on fixed regions (conditional) | Appendix: Global Constants Table (Provenance) | 1195-1201 | S0 | C0 | Updated (2025-12-25): now parallels `thm:nrc-quant` (includes the high-energy tail term and uses `assump:U2bR`). |
| YM-0049 | corollary | `cor:nrc-allz-fixed` | (Removed) AF--free NRC for all nonreal $z$ on fixed regions | Appendix: Global Constants Table (Provenance) | 1202-1208 | — | — | Removed from `Yang-Mills.tex` (2025-12-25) as redundant once the fixed-region comparison theorem was rewritten. |
...
| YM-0067 | theorem | `thm:global-os3-clustering` | Global OS3 (clustering) from a spectral gap (conditional) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / Global OS3 (clustering) and a physical mass scale | 1535-1541 | S0 | C0 | now a conditional corollary/pointer to `thm:global-gap-uncond` |
| YM-0068 | theorem | `thm:global-OS0-5` | Global OS0--OS5 with gap (conditional; any compact simple $G$) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / Global OS0--OS5 on $\mathbb R^4$ with positive mass gap | 1552-1558 | S0 | C0 | now a conditional pointer to `thm:global-os-clay` + `thm:global-gap-uncond` |
| YM-0069 | theorem | `thm:nrc-global-Oa2` | Global operator-norm NRC and projectors (conditional; see Clay pack) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / Uniform NRC and projectors in the large (global) | 1564-1574 | S0 | C0 | now a conditional pointer to `thm:global-nrc-clay` (removed stale “$O(a^2)$” rate claim) |
...
| YM-0071 | theorem | `thm:os-wightman-export` | OS$\to$Wightman export (conditional) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / OS$\to$Wightman export (gap, Poincar\'e, microcausality, nontriviality) | 1593-1595 | S0 | C0 | now a conditional pointer to `thm:gap-wightman-clay` / `thm:os-to-wightman` |
| YM-0072 | theorem | `thm:wightman-nongaussian` | Non-Gaussianity at the Wightman level (conditional) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / OS$\to$Wightman export (gap, Poincar\'e, microcausality, nontriviality) | 1600-1606 | S0 | C0 | included in the “nontriviality” clause of `thm:gap-wightman-clay` |
| YM-0073 | theorem | `thm:global-independence` | Unitary uniqueness and independence (conditional) | Global Continuum OS via Uniform Tightness and Isotropy (E1/E2) / Independence and group generality | 1620-1622 | S0 | C0 | now a conditional pointer to `thm:independence-clay` |
...
| YM-0143 | theorem | `thm:tightness-beta` | Tightness along $\beta(a)$; convergence conditional | Global continuum construction on $\mathbb R^4$ and OS axioms / Explicit AF-Style Scaling $\beta(a)$ and Tightness (optional; convergence conditional) | 2951-2953 | S0 | C0 | Updated (2025-12-25): statement now separates (i) tightness/precompactness from (ii) uniqueness/convergence, which is explicitly conditional on `prop:af-free-uniqueness` / `thm:U2-nrc-unique` (hence `assump:U2b`). |
| YM-0144 | corollary | `cor:global-convergence` | Convergence on $\mathbb R^4$ (conditional) | Global continuum construction on $\mathbb R^4$ and OS axioms / Explicit AF-Style Scaling $\beta(a)$ and Tightness (optional; convergence conditional) | 2958-2960 | S0 | C0 | Updated (2025-12-25): now explicitly conditional on fixed-region uniqueness/NRC inputs (no “tightness ⇒ uniqueness” overclaim). |
...
| YM-0146 | theorem | `thm:af-free-calibrated` | AF-free calibrated NRC and uniqueness (conditional; optional) | Global continuum construction on $\mathbb R^4$ and OS axioms / AF-Free Calibrated NRC Alternative (optional) | 2973-2975 | S0 | C0 | Updated (2025-12-25): explicitly labeled optional/not main chain (depends on a schedule-free one-step Doeblin hypothesis). |
...
| YM-0172 | theorem | `thm:c1-consolidated` | Consolidated continuum existence (optional Mosco cross-check; conditional) | Global continuum construction on $\mathbb R^4$ and OS axioms / Consolidated continuum existence (C1) | 3317-3326 | S0 | C0 | Updated (2025-12-25): removed an over-strong “unique (no subsequences)” claim; now only asserts subsequential limits, with uniqueness routed to `thm:U2-nrc-unique` (`assump:U2b`) in the AF-free chain. |
...
| YM-0200 | corollary | `lem:low-energy-proj` | Low--energy projector via contour | Norm--Resolvent Convergence via Embeddings and Resolvent Comparison / Cores and Consistency | 3830-3836 | S0 | C0 |  |
| YM-0200a | lemma | `lem:cell-avg-lowenergy` | Cell averaging is close to the identity on low energies | Norm--Resolvent Convergence via Embeddings and Resolvent Comparison / Cores and Consistency | 3837-3848 | S0 | C0 | Added (2025-12-25): supplies the comparison-side low-energy estimate for `lem:bridge-U2b-target` (so only (B1)–(B2) remain as the true bridge assumptions). |
...
| YM-0203 | corollary | `cor:unique-schwinger-local` | Unique Schwinger limits for local fields (conditional) | Norm--Resolvent Convergence via Embeddings and Resolvent Comparison / Cores and Consistency | 3888-3890 | S0 | C0 | Updated (2025-12-25): now explicitly conditional (depends on `thm:U2-nrc-unique` / `assump:U2b`), avoiding an unconditional “unique limit” overclaim. |
...
| YM-0209 | corollary | `cor:nonGaussian-main` | Non-Gaussianity of the continuum local fields (conditional) | Main Theorem (Continuum YM with Mass Gap; AF--free NRC with UEI/OS1 Inputs (RG-grade)) / Proof Strategy | 4039-4045 | S0 | C0 | Updated (2025-12-25): now explicitly conditional on the fixed-region uniqueness package (`cor:unique-schwinger-local` / `assump:U2b`), not on the old Appendix C1a statement. |
...
| YM-0221 | proposition | `prop:field-closability` | Field closability and relative graph bounds | Main Theorem (Continuum YM with Mass Gap; AF--free NRC with UEI/OS1 Inputs (RG-grade)) / Operator domains, common cores, and BRST / Closability and Graph Bounds for Smeared Fields | 4248-4255 | S0 | C0 | Updated (2025-12-25): proof no longer cites the optional calibrated-slab NRC template; uses uniform $L^\infty$ bounds of smeared plaquette/clover observables (bounded multiplication operators) to get the graph bound. |
...
| YM-0234 | lemma | `lem:nontrivial-variance` | Nontriviality: positive variance witness (conditional) | Continuum gauge symmetry, Gauss law, and BRST / Optional: BRST cohomology equals Gauss – law invariants | 4399-4409 | S0 | C0 | Updated (2025-12-25): rewritten as a short corollary of non-Gaussianity (`cor:nonGaussian-main`); removed the incorrect “uniform in $(\\beta,L)$ one-step Doeblin ⇒ uniform variance floor” overclaim. |
...
| ID | Type | Label | Title | Where (sec/subsec) | TeX lines | Status | Conf | Notes |
|---:|:-----|:------|:------|:------------------|:----------|:------:|:----:|:------|
| YM-0255 | theorem | `thm:c1a-tight` | Tightness of loop $n$-point functions; uniqueness conditional | Appendix: Tightness, convergence, and OS0/OS1 (C1a) | 5131-5133 | S0 | C0 | Updated (2025-12-25): removed the implied “NRC ⇒ $O(a)$ Cauchy ⇒ uniqueness” shortcut; uniqueness is now explicitly conditional on the AF--free operator-norm NRC/uniqueness package (`thm:U2-nrc-unique` / `assump:U2b`). |
...
### Optional: Asymptotic-freedom scaling schedule (conditional packaging; not used in main chain)

| ID | Type | Label | Title | Where (sec/subsec) | TeX lines | Status | Conf | Notes |
|---:|:-----|:------|:------|:------------------|:----------|:------:|:----:|:------|
| YM-0266 | lemma | `lem:cauchy-res` | (Removed) Cauchy estimate for embedded resolvents | Optional: Asymptotic-freedom scaling schedule (conditional packaging) | 5390-5395 | — | — | Removed from `Yang-Mills.tex` (2025-12-25): the standalone $O(a+b)$ Cauchy claim was redundant and hid missing projection/tail control; schedule-independence is now routed through `prop:one-point-resolvent` + `thm:U2-nrc-unique` (conditional on `assump:U2b`). |
| YM-0267 | theorem | `thm:af-schedule-gap` | AF schedule packaging (conditional; not used in main chain) | Optional: Asymptotic-freedom scaling schedule (conditional packaging) | 5406-5411 | S0 | C0 | Rewritten (2025-12-25) as a packaging restatement of the main conditional continuum construction (not an independent AF existence proof). |
...
| YM-0291 | lemma | `lem:U2-embeddings` | OS/GNS embeddings are genuine isometries | Appendix U: AF--free inputs and continuum limit (hypotheses U1--U4) / U2a. Embeddings and comparison identity | 5770-5772 | S3 | C4 | audited; sufficient for the “$I^*I=\mathrm{id}$” use in `thm:U2-nrc-unique` (2025-12-25) |
...
| YM-0294 | theorem | `thm:U2-nrc-unique` | AF–free uniqueness of the continuum generator | Appendix U: AF--free inputs and continuum limit (hypotheses U1--U4) / NRC via form approximation (abstract, quantified; Kato resolvent calculus) | 5815-5827 | S3 | C4 | audited; fixed sign error in the resolvent-identity bootstrap (2025-12-25) |
| YM-0295 | proposition | `prop:one-point-resolvent` | One–point resolvent estimate at a nonreal $z_0$ | Appendix U: AF--free inputs and continuum limit (hypotheses U1--U4) / NRC via form approximation (abstract, quantified; Kato resolvent calculus) | 5832-5844 | S3 | C4 | audited; proof now explicitly reduces to `thm:nrc-quant` so the projector term is not silently dropped (2025-12-25) |
| YM-0296 | lemma | `lem:U2-defect-core` | Defect identity and common core | Appendix U: AF--free inputs and continuum limit (hypotheses U1--U4) / NRC via form approximation (abstract, quantified; Kato resolvent calculus) | 5853-5863 | S3 | C4 | audited; removed incorrect $\int_0^\infty$ identity and corrected to the generator/derivative-at-$0^+$ identity (2025-12-25) |
