...
| 22 | `lem:scaled-ball-to-hk` | reduced (only remaining dependency is a now-precise external citation inside `lem:ballwalk-diffusive`) | `lem:ballwalk-diffusive` (cited) + Chernoff/Hoeffding + `lem:central-mixture-binomial` | The lemma is implemented in `Yang-Mills.tex` and reduces the Track B smoothing step to a diffusive-time Haar minorization for the uniform ball walk (`lem:ballwalk-diffusive`). Once that lemma is accepted (it is now cited precisely), binomial expansion for central mixtures + Haar absorption + a standard binomial tail bound yield the desired heat-kernel minorization uniformly for $n\gtrsim \beta$. |
| 23 | `lem:ballwalk-diffusive` | external theorem now precisely cited + hypotheses checked (in-text) | Hebisch--Saloff-Coste (1993), Theorem 5.1 (lower bound (15)), specialized to compact $G$ | `Yang-Mills.tex` now contains a repo-facing proof that reduces `lem:ballwalk-diffusive` to the Gaussian lower bound for convolution powers in \cite[Theorem 5.1]{HebischSaloffCoste1993} (using compactness to take growth order $D=0$ and estimating the word-metric diameter $\asymp r^{-1}$ to obtain a uniform lower bound at $n\asymp r^{-2}$). |
