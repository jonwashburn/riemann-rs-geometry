...
\end{lemma}
\begin{proof}[Proof sketch / placement]
This is the analytic core: a $\beta$--uniform lower bound for a fixed-radius small-ball event for a finite-dimensional Wilson conditional law on a cell. One route is via tree gauge, explicit one-link conditional laws of matrix-Fisher type, and a coarse “refresh” mechanism that does not lose mass as $\beta\to\infty$.
\end{proof}
...
By Lemma~\ref{lem:ucis-C-cell-ball}, in each cell the conditional law of the coarse increment dominates $p_*\,\nu_{r_*}$. Since factorization is exact and the pushforward under the (cellwise) coarse increment map preserves tensor products, the joint conditional law of the coarse increments dominates the product measure $p_*^{m(\varepsilon)}\,\nu_{r_*}^{\otimes m(\varepsilon)}$. This yields the stated product minorization.
\end{proof}
...
\end{lemma}
\begin{proof}
Let $R$ be the “refresh” kernel on $G^{m(\varepsilon)}$ defined by $R(U,\cdot):=\nu_{r_*}^{\otimes m(\varepsilon)}(\cdot)$ (independent of $U$). The hypothesis is exactly $K\ge \kappa R$ as positive kernels. By positivity and induction, for any $n\ge 1$,
\[
  K^{n}\ \ge\ \kappa^{n}\,R^{n}.
\]
