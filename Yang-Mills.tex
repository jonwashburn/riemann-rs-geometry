...
By the assumed near-identity refresh mechanism (implemented via Lemmas~\ref{lem:single-link-refresh}--\ref{lem:g-one-link-refresh} on each refreshed link within the cell, together with the cell locality ensured by Lemma~\ref{lem:ucis-A-cell-decomp}), there exist constants $\alpha_0,\kappa_0\in(0,1]$ independent of $(a,\beta,L)$ such that for all sufficiently small $a$,
\[
  \nu_{\beta(a)}\ \ge\ \alpha_0\,\mathrm{Unif}\!\big(B_G(e,\kappa_0/\sqrt{\beta(a)})\big).
\]
...
Using the scaling window \eqref{eq:ucis-sw-window}, choose $a$ sufficiently small so that $M(a)\ge C_0\,\beta(a)$ where $C_0$ is the constant from Lemma~\ref{lem:scaled-ball-to-hk} (depending only on $(G,\alpha_0,\kappa_0)$). Then there exist constants $c_0,t_0>0$ depending only on $(G,\alpha_0,\kappa_0)$ such that
\[
