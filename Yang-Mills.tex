...
\begin{lemma}[Local $O(a^2)$ E(4)\,--\,commutator bound on fixed regions (optional cross-check; uses Theorem~\ref{TS:sandwich_main})]\label{lem:local-commutator-Oa2}
...
\begin{itemize}
  \item[(i)] \textbf{(B2) as a form-level graph target.} Since $V_{a,L}$ is unitary, (B2) is equivalent to an $O(a)$ bound on the conjugated generator difference $V_{a,L}^*H^{\rm cmp}_aV_{a,L}-H_{a,L}$ relative to the $(H_{a,L}+1)^{1/2}$ graph norm. A typical way to state the desired analytic content is a bilinear \emph{operator-core} estimate: on a common core $\mathsf D_R\subset\mathrm{Dom}(H_{a,L})$ mapped by $V_{a,L}$ into $\mathrm{Dom}(H^{\rm cmp}_a)$,
  \[
...
\begin{itemize}
  ...
  \item[(D2)] \textbf{Form-level generator comparison (graph form; exact content of (B2)).} There exists $C_{\rm form}<\infty$ such that for all $\psi,\varphi$ in the common smooth cylinder core,
  \[
  ...
  \]
  where $\|\psi\|_{a,L}:=\|(H_{a,L}+1)^{1/2}\psi\|_{\mu_{a,L}}$. This is precisely the bilinear operator-core estimate displayed in Remark~(i) after Lemma~\ref{lem:bridge-U2b-target} when $V_{a,L}$ is the density isometry.
...
\end{itemize}
Under (D1)--(D3) (together with the graph-boundedness of $J_a$ and the graph-norm stability bounds recorded in Lemma~\ref{lem:bridge-U2b-target}), the bridge conditions (B1)--(B2) are reduced to a finite list of explicit density/form/geometry estimates on the tree-gauge cylinder core.
\end{lemma}
%
